...
| Notifications | Partial | `SignalASINotificationListenerService.kt` exposes bounded context and guarded `RemoteInput` reply | Register typed read/reply tools and add confirmation binding plus stale-notification tests |
...
| Camera, microphone, voice ASR/TTS | Partial | Manifest permissions, `LocalWhisperAsr.kt`, `WhisperModelManager.kt`, voice settings, and Android UI flows exist | Bind capture to sessions/artifacts; add explicit media tool descriptors and privacy lifecycle tests |
