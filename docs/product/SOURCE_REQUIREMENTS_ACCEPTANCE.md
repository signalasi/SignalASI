...
| CP-04 | Dedicated Codex, Claude Code, OpenClaw, cloud, local-model, Windows, and Android adapters | Generic transport adapter plus existing connector-specific execution paths | Partial | Replace connector-specific dispatch branches with named production adapters; add OpenClaw adapter |
| CP-05 | External Agents remain independently upgradeable and failure-isolated | Desktop CLI/App Server processes and HTTP connectors | Partial | Provider health isolation and crash-injection tests |
...
| CP-10 | Restore active Run, cursor, permission wait, tool state, and checkpoint after interruption | Run event store, workspace store, recovery policy | Partial | Reconnect a real remote Run and resume its event cursor after process death |
...
| CP-13 | Single-Agent and multi-Agent Provider access modes | `AgentAdapter`, `AgentProvider`, directory | Partial | Wire Desktop provider enumeration into the production directory |
...
| CP-16 | Durable idempotency for dangerous or repeatable operations | native-tool idempotency receipts; stable handoff identity | Partial | Persist native-tool replay receipts and remote Run start keys across process death |
...
| CP-19 | Persistent Runs include goal, plan, steps, results, permissions, device, tools, artifacts, errors, checkpoints, and handoffs | workspace/run stores and handoff ledger | Partial | One reconstruction test proving every field survives process death |
| CP-20 | Model, Agent, tool, Android, device, file, app, temporary, permanent, and consequential-action permission governance | host safety policy, native descriptors, confirmation policy | Partial | Unified permission grant ledger and revocation propagation gate |
...
| PA-03 | Every authorized semantic message, file, decision, task, tool result, and feedback event enters one event bus | global event bus and event publication policy | Partial | Complete publisher inventory and missing-event gate |
