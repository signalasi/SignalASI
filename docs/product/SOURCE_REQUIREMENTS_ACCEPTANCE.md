...
| CP-14 | Structured handoff with return path, artifacts, checkpoint, and reason | encrypted handoff ledger, production connector Run events, and two-process Android response-return instrumentation | Implemented | Paired Desktop handoff with naturally late artifacts |
| CP-15 | Background orchestration and user-visible Agent teams | guarded planner-graph compiler, stable host-owned Team Action, durable `AgentTeamExecutionRuntime`, production connector and one-response completion bridges, encrypted late-response correlation, duplicate suppression, single-responder policy, observer failure isolation, downstream dependency remapping, Recent Tasks expansion gate, and `test:android:team-process-death` forced-stop acceptance | Partial | Real paired-team dispatch with a naturally late Desktop response after forced Android process death |
...
| CP-18 | Real Android, Windows, file, terminal, Office, camera, microphone, notification, browser, and smart-home execution | live native-tool capability matrix, host-blocked invocation gate, bounded Android hardware/system/web tools, typed foreground camera and microphone artifact tools, redacted notification read/reply tools with stale-target guards, real-device inventory verification, typed Windows/file/terminal/Office Desktop tools with encrypted paired-phone routing, Desktop gateway, remote/declarative MCP, sandboxed Android-local stdio MCP, and Home Assistant connectors | Partial | Complete paired-phone real-device acceptance for Windows and Office plus one real-device verified operation for every remaining camera, microphone, notification, local stdio MCP, and smart-home class |
...
| PA-18 | Personal ASI can use native tools, Skills, MCP, specialist Agents, local/cloud models, Desktop, and smart-home resources | host registries, live executable capability matrix, remote/declarative and Android-local stdio MCP transports, `AgentTeamPlanCompiler`, `AgentProductionTeamController`, supervised specialist contracts, stable single-response team completion, and encrypted typed Desktop tool proxies with per-phone workspace isolation | Partial | One real-device scenario per non-phone resource class, including local stdio MCP, and paired multi-Agent synthesis |
