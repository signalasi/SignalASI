...
| CP-18 | Real Android, Windows, file, terminal, Office, camera, microphone, notification, browser, and smart-home execution | live native-tool capability matrix, host-blocked invocation gate, bounded Android hardware/system/web tools, typed foreground camera and microphone artifact tools, redacted notification read/reply tools with stale-target guards, typed Home Assistant connection/entity/service tools with encrypted credentials, per-entity consent, idempotency, and post-call controller-state verification, real-device inventory verification, Desktop gateway, and MCP connectors | Partial | Complete one real-device verified operation for every remaining Windows, Office, camera, microphone, notification, and smart-home class |
...
| PA-18 | Personal ASI can use native tools, Skills, MCP, specialist Agents, local/cloud models, Desktop, and smart-home resources | host registries, live executable capability matrix, typed Home Assistant tool projection, `AgentProductionTeamController`, supervised specialist contracts, and production connector team bridge | Partial | One real-device verified scenario per non-phone resource class and paired multi-Agent synthesis |
