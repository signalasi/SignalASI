...
| CP-18 | Real Android, Windows, file, terminal, Office, camera, microphone, notification, browser, and smart-home execution | live native-tool capability matrix, host-blocked invocation gate, bounded Android hardware/system/web tools, real-device inventory verification, Desktop gateway, MCP, and Home Assistant connectors | Partial | Complete one real-device verified operation for every remaining Windows, Office, camera, microphone, notification, and smart-home class |
