...
| PA-18 | Personal ASI can use native tools, Skills, MCP, specialist Agents, local/cloud models, Desktop, and smart-home resources | host registries, live executable capability matrix, `AgentTeamPlanCompiler`, `AgentProductionTeamController`, supervised specialist contracts, stable single-response team completion, and encrypted typed Desktop tool proxies with per-phone workspace isolation | Partial | One paired-phone real-device scenario per non-phone resource class and paired multi-Agent synthesis |
