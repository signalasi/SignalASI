...
| Cloud models | The app can add multiple providers as direct mobile model contacts; provider contact lists show provider names, while the chat header switches the selected model. | `npm run smoke:android:ui`, `npm run smoke:android:cloud-models` |
| Voice page | Local wake mode, ASR provider selection, PC STT routing, TTS settings, and reply preservation are available from the app. | `npm run smoke:android:ui`, `npm run smoke:android:voice-reply`, `npm run smoke:android:voice-settings`, `npm run smoke:desktop:voice-stt` |
